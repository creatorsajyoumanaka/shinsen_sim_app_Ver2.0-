...
# ※辞書は「同じキーがあると後勝ち」なので、重複キーを絶対に置かない
STATUS_KEYWORDS = {
    "威圧": "stun",
    "麻痺": "paralyze",
    "混乱": "confuse",
    "無策": "silence",
    "封撃": "disarm",   # 「通常攻撃不可(確率/減衰)」は seal_attack を別途付与する
    "火傷": "burn",
    "水攻め": "flood",
    "潰走": "collapse",
    "消沈": "depress",
...
def _prob_to_p(base_prob: float) -> float:
    """
    base_prob の表現揺れ吸収：
      - 35.0 -> 0.35
      - 0.35 -> 0.35
...
class Skill:
    ...
    base_prob: float = 0.0  # percent or 0..1 (engine will normalize)
    level: int = 20
    awaken: bool = True
...
def _apply_opening_effects(allies: List[Unit], enemies: List[Unit], rng: random.Random, logs: List[LogRow]):
    """
    開幕で passive / command のうち「それっぽい効果」を一回だけ適用する。
    - 受動：連撃 / 武勇+X
    - 気炎万丈：封撃（通常攻撃不可：確率/減衰）を敵複数に付与
    """
    for unit in allies + enemies:
        ...
        for sk in [unit.unique_skill] + unit.inherited:
            ...
            # ---- Command（指揮）: 封撃（通常攻撃不可）付与（気炎万丈想定） ----
            if (sk.name == "気炎万丈") or ((sk.kind == "command" or "指揮" in raw) and ("封撃" in raw) and ("通常攻撃不可" in raw)):
                ...
                for t in targets:
                    # 3ターン目まで：turn1..3 で判定されるよう turns=3
...
def _try_cast_skill(
    ...
    NOTE:
      - passive/command は「自動適用」扱いで action 消費しないように False を返す
    """
    raw = skill.raw or ""

    # ---- Passive/Command special effects: 自動適用（ターン消費しない） ----
    if raw:
        # 受動：連撃 / 武勇+X（成田固有など）
        if skill.kind == "passive" or "受動" in raw:
            ...
            if m and "bonus_wu" not in unit.statuses:
                unit.wu += int(m.group(1))
                unit.statuses["bonus_wu"] = {"turns": 999999}
            # passive はここで終わり（行動消費なし）
            return False, "（受動: 自動適用）", []

        # 指揮：気炎万丈（封撃の確率付与）を 1回だけ付与
        if (skill.name == "気炎万丈") or ((skill.kind == "command" or "指揮" in raw) and ("封撃" in raw) and ("通常攻撃不可" in raw)):
...
def simulate_battle(
    ...
    for turn in range(1, turns + 1):
        ...
        # ターン終了時にまとめて status tick（ここで1回だけ）
        for u in allies + enemies:
            if u.alive():
                _tick_statuses(u)
